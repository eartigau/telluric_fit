...
def main(instrument: str = 'NIRPS', doplot: bool = None, n_cores: int = None):
    ...
    if parallel_enabled:
        ...
        while remaining_args:
            ...
            try:
                async_results = [(idx, arg, pool.apply_async(_process_wrapper, (arg,)))
                                 for idx, arg in remaining_args]
                pool.close()

                deadline = time.perf_counter() + TASK_TIMEOUT
                completed_this_round = set()
                hung = False

                while len(completed_this_round) < len(async_results):
                    now = time.perf_counter()
                    prev_done = len(completed_this_round)
                    for idx, arg, r in async_results:
                        if idx in completed_this_round:
                            continue
                        try:
                            result = r.get(timeout=0)
                            completed_this_round.add(idx)
                            n_done += 1
                            if result:
                                n_processed += 1
                            else:
                            ...
                            remaining_count = N_pending - n_done
                            ...
                            deadline = time.perf_counter() + TASK_TIMEOUT  # reset on progress
